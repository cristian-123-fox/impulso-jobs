...
> ⚠️ El `PermissionsGuard` lee los permisos **de la base de datos**. Agregar un código a `seed-rbac.ts` no basta: hay que **volver a correr `pnpm seed`**, o el endpoint responderá 403.
...
- **`account.data_export` se añadió en M13**, así que hace falta haber corrido `pnpm seed` después de esa versión o `GET /account/data-export` responderá 403.
...
- **Los permisos `subscriptions.*` se añadieron en M14**: hace falta volver a correr `pnpm seed` tras desplegar esa versión.

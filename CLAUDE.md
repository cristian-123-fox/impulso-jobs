...
- **Backend** — `modules/`: `iam/{auth,users,roles,permissions,registration,account}`, `candidates`, `companies`, `vacancies`, `applications`, `talent`, `billing`, `audit`. 24 controllers, 49 use-cases, 13 migrations, 33 unit specs. Swagger at `/docs` + `/docs-json`, global prefix `/api/v1`.
- **Not built** — the **Stripe adapter** (billing itself is done; payments go through `PaymentProviderPort`, today bound to `ManualPaymentAdapter`), notifications, screening questions (`vacancy_questions` + `application_answers`), CFDI, AI. M11–M14 have no frontend yet. No e2e tests yet.

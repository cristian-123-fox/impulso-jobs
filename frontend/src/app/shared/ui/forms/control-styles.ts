...
export const IJ_LABEL = 'mb-2 block text-sm font-semibold text-ink-900';
export const IJ_HINT = 'mt-1 text-[12px] text-muted';
export const IJ_ERROR = 'mt-1 text-[12px] font-medium text-red-600';

/** Caja base de un control (input o disparador de dropdown). */
const CONTROL_BASE =
  'flex min-h-[46px] w-full items-center gap-2 rounded-xl border bg-white px-3.5 ' +
  'text-[14px] text-ink-900 transition-colors outline-none';
...
export function ijControlClass(options: {
  ...
  const { focused, invalid, disabled } = options;
  let state: string;
  if (disabled) {
    ...
    state = 'border-line hover:border-brand/40';
  }
  return `${CONTROL_BASE} ${state}`;
}

/** Clase para un `<input>`/`<textarea>` nativo dentro del control. */
export const IJ_NATIVE_INPUT =
  'w-full bg-transparent py-2.5 text-[14px] text-ink-900 placeholder:text-muted outline-none';
...
export const IJ_SEARCH =
  'mb-1 w-full border-b border-line px-2.5 pb-2.5 pt-1 text-[13.5px] text-ink-900 placeholder:text-muted outline-none';
